...
object GuildDBController {
    ...
    fun guildVolume(id: Long, volume: Int) {
        val ps = Connection().prepareStatement("update guild set volume = ? where id = ?")
        ps.setInt(1, volume)
        ps.setLong(1, id)
        ps.execute()
    ...
    fun addGuild(guildId: Long) {
        val ps = Connection().prepareStatement("insert into guild values (?, ?);")
    ...
    fun delGuild(guildId: Long) {
        val ps = Connection().prepareStatement("delete from guild where id=?")
        ps.setLong(1, guildId)
        ps.execute()
    ...
    fun checkGuild(guildId: Long): Boolean {
        val ps = Connection().prepareStatement("select exists(select * from guild where id=?)")
        ps.setLong(1, guildId)
