...
import com.github.yeoj34760.spuppybot.TrackManager.base64ToTrack
import com.github.yeoj34760.spuppybot.db.DB.Connection
import com.github.yeoj34760.spuppybot.db.`class`.UserBox

object UserBoxDBController {
    fun addUserBox(id: Long, track: String) {
        val ps = Connection().prepareStatement("insert into user_box values (?,?,?)")
        ps.setLong(1, id)
    ...
    fun delAllUserBox(id: Long) {
        val ps = Connection().prepareStatement("delete from user_box where id=?")
        ps.setLong(1, id)
        ps.execute()
    }
    ...
    fun checkUserBox(id: Long): Boolean {
        val ps = Connection().prepareStatement("select exists(select * from user_box where id=?)")
        ps.setLong(1, id)
    ...
    fun fromUserBox(id: Long): List<UserBox> {
        ...
        while (t.next()) {
            ...
            tempBox.add(UserBox(id, base64ToTrack(t.getString(1)), order))
        }
