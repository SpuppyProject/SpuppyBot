...
object ReceiveMoneyDBController {
    ...
    fun receiveMoneyTimer(id: Long): Long {
        val ps = Connection().prepareStatement("select timer from user_receive_money_timer where id=?")
        ps.setLong(1, id)
    ...
    fun checkReceiveMoneyUser(id: Long): Boolean {
        val ps = Connection().prepareStatement("select exists(select * from user_receive_money_timer where id=?)")
        ps.setLong(1, id)
