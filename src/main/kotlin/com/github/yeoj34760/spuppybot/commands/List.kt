...
object List : Command() {
    ...
    override fun execute(event: CommandEvent?) {
        ...
        GuildManager.get(id).trackQueue.forEach {
            list += "${i++}. [${it.info.title}](${it.info.uri})\n"
        }
        val embed = EmbedBuilder()
                .setTitle("SppuppyBot")
                .setThumbnail(Util.youtubeToThumbnail(GuildManager.get(id).playingTrack().info.identifier))
                .setDescription("재생중 -> [${GuildManager.get(id).playingTrack().info.title}](${GuildManager.get(id).playingTrack().info.uri})")
